...
struct AMP_Proto
{
    ...
    unsigned char key_data[256+1];
    int key_data_fetched;
...
# 397 "amp.h"
int amp_consume_bytes(AMP_Proto_p proto, unsigned char* buf, int nbytes);
...
# 468 "amp.h"
int amp_respond_error(AMP_Proto_p proto, AMP_Request_p request,
...
# 490 "amp.h"
AMP_Box_p amp_new_box_custom(int hint, key_cmp_func *cmp, key_hash_func *hash);
...
# 574 "amp.h"
int amp_get_bytes(AMP_Box_p box, const char *key, unsigned char **buf, int *size);
...
int amp_put_bytes(AMP_Box_p box, const char *key, const unsigned char *buf, int size);
...
int amp_get_long_long(AMP_Box_p box, const char *key, long long *value);


int amp_put_long_long(AMP_Box_p box, const char *key, long long value);
...
int amp_put_cstring(AMP_Box_p box, const char *key, const unsigned char *value)
{
    return _amp_put_buf(box, key, value, strlen(value));
}
...
int amp_put_bool(AMP_Box_p box, const char *key,
                 int value)
{
    const char *buf;
    if (value)
    ...
    return _amp_put_buf(box, key, buf, strlen(buf));
}



int amp_get_bool(AMP_Box_p box, const char *key,
                 int *value)
{
    int err;
    unsigned char *buf;
...
{
# 107 "types.c"
    int bufLen = ((int)(log10(pow(2, sizeof(long long)*8)))) + 3;
    char buf[bufLen];
    int result;

    result = __builtin___snprintf_chk (buf, bufLen, 0, __builtin_object_size (buf, 2 > 1), "%lld", value);
...
{
# 154 "types.c"
    int bufLen = ((int)(log10(pow(2, sizeof(int)*8)))) + 3;
    char buf[bufLen];
    int result;

    result = __builtin___snprintf_chk (buf, bufLen, 0, __builtin_object_size (buf, 2 > 1), "%d", value);
    if (result < 0 || result >= bufLen)
    {
...
int amp_put_uint(AMP_Box_p box, const char *key, unsigned int value)
{
    ...
    result = __builtin___snprintf_chk (buf, bufLen, 0, __builtin_object_size (buf, 2 > 1), "%u", value);
    if (result < 0 || result >= bufLen)
    {
...
int amp_put_double(AMP_Box_p box, const char *key, double value)
{
    ...
    if (( sizeof (value) == sizeof(float ) ? __inline_isnanf((float)(value)) : sizeof (value) == sizeof(double) ? __inline_isnand((double)(value)) : __inline_isnan ((long double)(value))))
    {
        buf_size = 3;
        ((__builtin_object_size (buf, 0) != (size_t) -1) ? __builtin___strncpy_chk (buf, "nan", buf_size, __builtin_object_size (buf, 2 > 1)) : __inline_strncpy_chk (buf, "nan", buf_size));
    }
    else if (( sizeof (value) == sizeof(float ) ? __inline_isinff((float)(value)) : sizeof (value) == sizeof(double) ? __inline_isinfd((double)(value)) : __inline_isinf ((long double)(value))))
    {
        if (( sizeof (value) == sizeof(float ) ? __inline_signbitf((float)(value)) : sizeof (value) == sizeof(double) ? __inline_signbitd((double)(value)) : __inline_signbit((long double)(value))) == 0)
        {

            buf_size = 3;
            ((__builtin_object_size (buf, 0) != (size_t) -1) ? __builtin___strncpy_chk (buf, "inf", buf_size, __builtin_object_size (buf, 2 > 1)) : __inline_strncpy_chk (buf, "inf", buf_size));
        }
        else
        {
    ...
    else
    {
        ...
        buf_size = __builtin___snprintf_chk (buf, 100, 0, __builtin_object_size (buf, 2 > 1), "%.17f", value);
    }
...
int amp_get_double(AMP_Box_p box, const char *key, double *value)
{
    ...
    if (size == 3)
    {
        ...
        else if (strncmp(buf, "nan", 3) == 0)
        {

            *value = __builtin_nanf("0x7fc00000");
            return 0;
        }
    }
    else if (size == 4 && strncmp(buf, "-inf", 4) == 0)
    {

            *value = __builtin_huge_valf() * -1;
            return 0;
    }
